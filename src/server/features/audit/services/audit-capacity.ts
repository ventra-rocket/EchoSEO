...
export const MAX_AUDIT_PAGES = AUDIT_MAX_PAGES;

/** Omitting a page count means "crawl the site", not "crawl fifty pages". */
export function clampAuditMaxPages(maxPages?: number) {
  return Math.min(
    Math.max(maxPages ?? AUDIT_MAX_PAGES, AUDIT_MIN_PAGES),
    AUDIT_MAX_PAGES,
  );
}
