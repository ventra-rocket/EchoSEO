 ...
 *
 * Three things age out, on different clocks:
...
export async function sweepAuditRetention(
  ...
  if (exportsExpired === 0 && buildsTerminalized === 0 && auditsPurged === 0) {
    ...
    console.log(
      `[cron] audit retention: expired ${exportsExpired} export(s), terminalized ${buildsTerminalized} build(s), purged ${auditsPurged} audit(s)`,
    );
  }

  return { exportsExpired, buildsTerminalized, auditsPurged };
}
...
async function purgeAuditsPastRetention(): Promise<number> {
  ...
  // Both the export ZIPs and the per-page Lighthouse payloads live in R2 and are
  // only referenced by rows the cascade is about to remove — purge them first or
  // they orphan permanently.
  const [exportKeys, lighthouseKeys] = await Promise.all([
    AuditRetentionRepository.findExportKeysForAudits(auditIds),
    AuditRetentionRepository.findLighthouseKeysForAudits(auditIds),
  ]);
  const keys = [...exportKeys, ...lighthouseKeys];
  if (keys.length > 0) {
