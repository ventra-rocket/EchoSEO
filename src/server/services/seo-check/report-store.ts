...
/**
 * Purges report payloads by key (retention sweep), returning any key it could
 * not delete so the caller can log precisely what leaked.
 *
 * Chunked to stay under R2's per-call key cap: a busy backlog drain can hand
 * over more than one call's worth. A batch is caught rather than thrown so one
 * bad call cannot skip the rest — the caller deletes the D1 rows either way,
 * after which a missed key can never be derived again and the object is
 * orphaned for good.
 */
