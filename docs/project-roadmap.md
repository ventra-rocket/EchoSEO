...
**Version:** 1.0 · **Date:** 2026-08-22 · **Status:** M0 complete. Free SEO Checker and the Professional Site Audit are deployed in production, and the Google-First Parity Sprint (13–31/08) delivered all 12 committed days. **The app is now fully bilingual VN + EN: 2,026 react-intl ids per catalog, every shipped surface converted and verified in a browser in both locales** (#97, #98, #112, #114, #116 — see the i18n line under MVP for the exact waves and the two tracked gaps). Since 19/08: Search Console actuals overlay tracked keywords, and the last hosted-vendor dependency (`@every-app/sdk`) is gone. Billing remains unlaunched. Remaining MVP work is dogfooding the inherited surfaces and the public OSS release; the content engine and read-write agent loop are unbuilt V1 goals. Open issues: #107, #108, #109, #111.
...
- [x] **Bilingual VN + EN across all shipped surfaces** — 2,026 react-intl ids per catalog, key parity enforced by types and pinned by `messages.test.ts`. Completed 22/08 over four waves: Site Audit and the shell earlier, then Rank Tracking / Search Performance / Keyword Research / Saved Keywords (#97, #98), the pre-login path (#112 — sign-in/sign-up, verification, password reset, OAuth consent, onboarding wizard and strategy chat), Domain Overview and Backlinks (#114), and finally AI visibility, projects/settings, billing and Lighthouse/help/support (#116). Three of those waves found the same class of defect that the directory-shaped prose gate cannot see, because the English lived in code rather than copy: `useAccessGate` resolving provider failures through an English-only error map (four surfaces already listed as bilingual), `features/ai-mcp` rendering half the `/ai` page from an unlisted directory, and locale-blind formatters (`new Intl.NumberFormat("en-US")`, `toLocaleDateString()`, `` `$${value}` ``) that follow the OS or a hardcoded locale instead of the reader's. 32 legacy-resolver callsites were migrated; the gate now lists every shared directory a converted surface renders from. **Two known gaps, both tracked:** the onboarding assistant's own replies are still English because its system prompt carries no locale (#111), and the server SEO rule catalogue is disclosed as English rather than translated pending a decision (#107). Vietnamese remains a machine-translated seed pending human review (`src/client/i18n/messages/README.md`).
