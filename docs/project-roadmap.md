...
**Version:** 1.1 · **Date:** 2026-08-23 · **Status:** M0 complete. Free SEO Checker and the Professional Site Audit are deployed in production, and the Google-First Parity Sprint (13–31/08) delivered all 12 committed days. **The app is fully bilingual VN + EN with no remaining exceptions: 2,026 react-intl ids per catalog, every shipped surface verified in a browser in both locales, every rule catalogue translated, and the onboarding assistant now answering in the reader's language** (#97, #98, #112, #114, #116, #118, #119). Since 19/08: Search Console actuals overlay tracked keywords, and the last hosted-vendor dependency (`@every-app/sdk`) is gone. Billing remains unlaunched. Remaining MVP work is dogfooding the inherited surfaces and the public OSS release; the content engine and read-write agent loop are unbuilt V1 goals. Open issues: #108 (weekly email needs a human to open it in three mail clients), #109 (one unreproduced `test:ci` exit code), #121 (production has no `OPENROUTER_API_KEY`, latent until open access closes).
...
- [x] **Bilingual VN + EN across all shipped surfaces** — 2,026 react-intl ids per catalog, key parity enforced by types and pinned by `messages.test.ts`. Completed 22/08 over six waves: Site Audit and the shell earlier, then Rank Tracking / Search Performance / Keyword Research / Saved Keywords (#97, #98), the pre-login path (#112), Domain Overview and Backlinks (#114), AI visibility / projects / billing / Lighthouse-help-support (#116), and finally the two paths no prose gate could see — the onboarding assistant's own replies, which had no locale in its system prompt (#118, verified against the production model), and the cross-page rule catalogue, whose four rules rendered English behind a "(guidance shown in English)" disclosure (#119). **No exceptions remain.** Four of those waves found the same defect class: the English lived in code rather than copy, where a directory-shaped or hand-enumerated gate cannot reach it — `useAccessGate` resolving provider failures through an English-only map, `features/ai-mcp` rendering half the `/ai` page from an unlisted directory, `new Intl.NumberFormat("en-US")` and `toLocaleDateString()` following the OS instead of the reader, a `statusErrorFallback` passed as an object-literal property, and a rule catalogue in a directory the localization test never listed. 32 legacy-resolver callsites migrated; every shared directory a converted surface renders from is now gated, and the rule-locale test covers all five catalogues. Vietnamese remains a machine-translated seed pending human review (`src/client/i18n/messages/README.md`).
