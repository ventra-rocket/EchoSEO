...
**Version:** 0.8 · **Date:** 2026-08-21 · **Status:** M0 complete. Free SEO Checker and the Professional Site Audit are deployed in production, and the Google-First Parity Sprint (13–31/08) delivered all 12 committed days: weekly report email, trust repair on the free report, GSC multi-site import, site dashboard cards, page-level competitor audit, and a measured 5,000-page crawl ceiling. Every GitHub issue is closed. Since 19/08: Search Console actuals now overlay tracked keywords; the authenticated i18n pass covers Site Audit, Rank Tracking, Search Performance, Keyword Research, Saved Keywords and their shared app/GSC states; and the last hosted-vendor dependency (`@every-app/sdk`) is gone. Billing remains unlaunched. Remaining MVP work is dogfooding + hardening the inherited domain, backlink and AI-visibility surfaces, finishing their VN/EN copy, and the public OSS release; the content engine and read-write agent loop are unbuilt V1 goals.
...
- [~] **Bilingual VN + EN** across all shipped surfaces — 1,215 react-intl ids now cover the shell, Command Center, Site Audit, Rank Tracking, Search Performance, Keyword Research, Saved Keywords and shared app/GSC states; domain overview, backlinks, brand lookup, prompt explorer and the AI workspace remain. See the i18n note at the top for the exact split and verification.
