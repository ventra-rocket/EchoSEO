...
- [~] **Bilingual VN + EN** across all shipped surfaces — 1,420 react-intl ids now cover the shell, Command Center, Site Audit, Rank Tracking, Search Performance, Keyword Research, Saved Keywords, shared app/GSC states, and the whole pre-login path (sign-in/sign-up, verification, password reset, OAuth consent, onboarding wizard and strategy chat, shipped 22/08 in #112). Remaining English surfaces are tracked as issues: domain overview (#101), backlinks (#102), brand lookup / prompt explorer / AI workspace (#103), projects and settings (#104), billing (#105), Lighthouse / help / support (#106). The onboarding assistant's own replies are still English — its system prompt carries no locale (#111). See the i18n note at the top for the exact split and verification.
