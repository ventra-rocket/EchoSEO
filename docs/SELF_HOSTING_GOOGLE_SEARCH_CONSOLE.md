...
Connecting Google Search Console (GSC) lets EchoSEO pull your real clicks,
impressions, positions, and URL inspection data, straight from Google.

It's **optional**: EchoSEO runs fine without it, just without Search Console data.
...
2. Add an **Authorized redirect URI** that exactly matches your deployment's
   ...
   | ------------ | -------------------------------------------------------- |
   | Deployed     | `https://your-echoseo-domain.com/api/gsc/oauth/callback` |
...
Set these three values, then restart EchoSEO:
...
Restart EchoSEO so it picks up the new variables. For Docker, changing `.env`
...
- EchoSEO uses your Google client to run the OAuth flow and stores the resulting
...
- Search Console data comes from your own Google account, so EchoSEO never meters credits for it.
